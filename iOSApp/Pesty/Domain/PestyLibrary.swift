...
struct PestyLibrary: Codable, Sendable {
    ...
    mutating func applyRemoteDeletion(id: UUID, at date: Date = .now) {
        applyRemoteDeletions(ids: [id], at: date)
    }

    /// Applies a CloudKit deletion batch in linear passes. Calling the
    /// single-record implementation once per ID repeatedly scanned every clip
    /// and Pinboard, which became noticeable during a large manual refresh.
    mutating func applyRemoteDeletions(ids: [UUID], at date: Date = .now) {
        let deletedIDs = Set(ids)
        guard !deletedIDs.isEmpty else { return }

        for clipIndex in clips.indices where deletedIDs.contains(clips[clipIndex].id) {
            let deletionDate = Self.timestampStrictlyAfter(clips[clipIndex].updatedAt, preferred: date)
            clips[clipIndex].deletedAt = deletionDate
            clips[clipIndex].updatedAt = deletionDate
        ...
        for boardIndex in boards.indices {
            boards[boardIndex].clipIDs.removeAll { deletedIDs.contains($0) }
            boards[boardIndex].pinnedClipIDs.removeAll { deletedIDs.contains($0) }
        }

        let deletedBoardIDs = Set(boards.lazy.filter { deletedIDs.contains($0.id) }.map(\.id))
        for boardIndex in boards.indices where deletedBoardIDs.contains(boards[boardIndex].id) {
        ...
        let boardDeletionDates = Dictionary(uniqueKeysWithValues: boards.compactMap { board in
            board.deletedAt.map { (board.id, $0) }
        })
        for clipIndex in clips.indices {
            guard let containerID = clips[clipIndex].containerID,
                  deletedBoardIDs.contains(containerID) else { continue }
            let boardDeletionDate = boardDeletionDates[containerID] ?? date
            let clipDeletionDate = Self.timestampStrictlyAfter(
                clips[clipIndex].updatedAt,
                preferred: boardDeletionDate
            )
            clips[clipIndex].deletedAt = clipDeletionDate
            clips[clipIndex].updatedAt = clipDeletionDate
            clips[clipIndex].preDeletionUpdatedAt = nil
            clips[clipIndex].deletionFinalizedAt = clipDeletionDate
            updatedAt = max(updatedAt, clipDeletionDate)
        }
    }
