...
final class ClipboardMonitor {
    ...
    private func makeItem() -> ClipItem? {
        let declaredSourceID = PasteboardSourceMarker.identifier(on: pasteboard)
        // Pesty updates the clipboard for Copy, Edit, and Paste. Those writes
        // are handled explicitly by their initiating flows, so never capture
        // them again as a new history item or Paste Stack entry if a monitor
        // poll lands on an intermediate pasteboard change count.
        guard !PasteboardSourceMarker.isPestyOrigin(declaredSourceID) else { return nil }
        ...
        // A source marker identifies content that another clipboard manager
        // restored without confusing it with whichever app happens to be
        // frontmost at polling time. Do not fall back to that unrelated app's
        // name when the marked bundle is no longer installed.
        let bundleID = declaredSourceID ?? sourceApp?.bundleIdentifier
        let appName = declaredSourceID.flatMap(applicationName(forBundleID:)) ?? sourceApp?.localizedName
