...
final class BarHeightGhostController {
    ...
    /// The ghost renders a real `BarView`, which needs a search bridge. It
    /// gets its own rather than sharing the live bar's, so the preview can
    /// never be mistaken for the real field owning focus.
    ...
    func show(height: CGFloat) {
        ...
        panel.setFrame(NSRect(x: visible.minX, y: visible.minY, width: visible.width, height: height),
                       display: true)
        (panel.contentView as? NSHostingView<BarHeightGhostView>)?.rootView =
            BarHeightGhostView(height: height, searchBridge: searchBridge)
    ...
    private func makePanel() -> NSPanel {
        ...
        panel.contentView = NSHostingView(
            rootView: BarHeightGhostView(height: 0, searchBridge: searchBridge))
        return panel
    }
}
...
private struct BarHeightGhostView: View {
    ...
    var body: some View {
        ZStack {
            ...
            BarView(searchBridge: searchBridge)
